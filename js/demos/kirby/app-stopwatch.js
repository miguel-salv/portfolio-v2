...
import {
  ...
  KIRBY_ALERT_TOP, KIRBY_ALERT_BOT, KIRBY_TEXT as TEXT,
} from "./theme.js";
...
export function createStopwatchApp() {
  ...
  horizGradBg(swCont);
  stars(swCont);
  const swLbl = label(swCont, "00:00.00", { font: FONT_LARGE, color: KIRBY_TEXT, center: true, y: 88 });
  ...
  horizGradBg(tmrCont);
  stars(tmrCont);
  label(tmrCont, "Min", { font: FONT_SMALL, color: KIRBY_ACCENT_WARM, x: 98, y: 24 });
  label(tmrCont, "Sec", { font: FONT_SMALL, color: KIRBY_ACCENT_WARM, x: 192, y: 24 });

  const rollersCont = el("div", "kirby-tmr-rollers");
  const minR = miniRoller(rollersCont, 100, tmrMin, (v) => { tmrMin = v; });
  minR.style.left = "76px";
  label(rollersCont, ":", { font: FONT_LARGE, color: KIRBY_TEXT, x: 154, y: 44 });
  const secR = miniRoller(rollersCont, 60, tmrSec, (v) => { tmrSec = v; });
  secR.style.left = "172px";
  tmrCont.appendChild(rollersCont);

  const countdownLbl = label(tmrCont, "00:05", { font: FONT_LARGE, color: KIRBY_TEXT, center: true, y: 90 });
  ...
  label(alertCont, "Time's up!", { font: FONT_LARGE, color: TEXT, center: true, y: 72 });
